...
export class AppComponent implements OnInit {
  ...
  private updateTick(frame:number) {
    ...
    this.players = this.players.map(player => {
      ...
      player.force = this.calcForce(player.force);
      player.acceleration = this.calcAcceleration(player.force, player.mass);
      player.velocity = this.calcVelocity(player.acceleration);
   ...
   */
  private calcVelocity(acceleration: [number, number]): [number, number] {
    return [
      acceleration[0] * (1000/60),
      acceleration[1] * (1000/60),
    ];
  }

  /**
   * A function representing the physics formula A = F * m. Calculates the new acceleration vector based on their current force and mass.
   * @param force The Force vector being applied to the player. This can result from user input, friction, and collisions.
   * @param mass The mass of the player
   * @returns An acceleration vector.
   */
  private calcAcceleration(force:[number, number], mass: number): [number, number] {
    return [
      force[0] / mass,
      force[1] / mass,
    ];
   ...
   */
  private calcForce(currentForce:[number, number]): [number, number] {
  ...
  private checkCollisions(currentPlayer: Player): Player {
    ...
    if(
      ...
      currentPlayer.acceleration[0] *= -this.gameSettings.bounceCoefficient;
      currentPlayer.force[0] *= -this.gameSettings.bounceCoefficient;
    }
    ...
    if(
      ...
      currentPlayer.acceleration[1] *= -this.gameSettings.bounceCoefficient;
      currentPlayer.force[1] *= -this.gameSettings.bounceCoefficient;
    }
